...
namespace allN1
{
...

    {
        ...
        {
            txtid.Text = users.useridSelected;
            txtuser.Text = users.usernameSelected;
        ...
        private void BillList_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                Quary.Append("select total_price  , (strftime('%d/%m/%Y',date)) as date from orders where order_id = ");
                Quary.Append(billList.SelectedValue);
                
                using (connection = new SQLiteConnection(connectionString))
                using (SQLiteCommand command = new SQLiteCommand(Quary.ToString(), connection))
        ...
        private void delete()
        {
            String quary = "delete from  orders where order_id = @order_id  ";
        ...
        private void Button2_Click(object sender, EventArgs e)
        {
           ...
           if (dataGridView1.CurrentCell == null)
            {
                MessageBox.Show("قم بتحديد الصنف أولا");
            }
            else
            {
                try
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!Directory.Exists(Application.StartupPath + "/bills/"))
                {
                    DirectoryInfo di = Directory.CreateDirectory(Application.StartupPath + "/bills/");
                }
            }
            catch (Exception ex)
            {
